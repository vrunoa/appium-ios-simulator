...
async function update (pathToPlist, updates, clear = false) {
  let currentSettings = {};
  if (clear === false) {
    currentSettings = await read(pathToPlist);
  }
...
async function setSafariLiteSettings (sim) {
  let safariSettings = {};
  safariSettings.DidMigrateYouTubeBookmarks = true;
  safariSettings.DidImportBuiltinBookmarks = true;
  safariSettings.ShouldCheckHistoryStoreDatabaseIntegrity = false;
  safariSettings.SearchEngineStringSetting = '';
  safariSettings.SpotlightSuggestionsEnabled = false;
  safariSettings.SuppressSearchSuggestions = true;
  safariSettings.ShowTabBar = false;
  safariSettings.OpenLinksInBackground = false;
  safariSettings.SpeculativeLoading = false;
  safariSettings.HideFrequentlyVisitedSites = true;
  safariSettings.DisableWebsiteSpecificSearch = true;
  // actually do the update
  let updated = false;
  log.debug(
    'Setting Safari settings to lite to improve reliability',
    JSON.stringify(safariSettings)
  );

  const paths = await plistPaths(sim, 'mobileSafari');
  for (const file of paths) {
    updated = await update(file, safariSettings, true);
  }
...
export {
  ...
  setSafariLiteSettings, plistPaths
};
